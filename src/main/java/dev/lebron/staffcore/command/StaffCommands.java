package dev.lebron.staffcore.command;
...
import dev.lebron.staffcore.gui.menu.AltsMenu;
import dev.lebron.staffcore.gui.menu.AppealsMenu;
import dev.lebron.staffcore.gui.menu.ContrabandMenu;
import dev.lebron.staffcore.gui.menu.VaultMenu;
import dev.lebron.staffcore.gui.menu.EnderChestMenu;
import dev.lebron.staffcore.gui.menu.InvseeMenu;
import dev.lebron.staffcore.gui.menu.LogsMenu;
import dev.lebron.staffcore.gui.menu.PlayerActionsMenu;
import dev.lebron.staffcore.gui.menu.ReportsMenu;
import dev.lebron.staffcore.gui.menu.SecurityMenu;
import dev.lebron.staffcore.gui.menu.StaffPanelMenu;
import dev.lebron.staffcore.module.Mods;
import dev.lebron.staffcore.modules.grief.GriefModule;
import dev.lebron.staffcore.modules.grief.LogQuery;
...
import dev.lebron.staffcore.permission.PermissionGroups;
...
public final class StaffCommands {
	...
	private static int status(CommandContext<CommandSourceStack> ctx) {
		...
		var findings = dev.lebron.staffcore.diagnostic.StartupCheck.report();
	...
	private static int inspectHeld(CommandContext<CommandSourceStack> ctx) {
		...
		dev.lebron.staffcore.gui.menu.ItemDetailsMenu.open(player, held, "your hand", null);
		return 1;
	}
	...
	private static int selfTest(CommandContext<CommandSourceStack> ctx) {
		...
		var results = dev.lebron.staffcore.diagnostic.SelfTest.run(server);
	...
	private static int antiCheatTest(CommandContext<CommandSourceStack> ctx, int confidence) {
		...
		Mods.antiCheat().report("StaffCore-test", name,
				dev.lebron.staffcore.modules.anticheat.AntiCheatEvent.Kind.DETECTION,
				"SelfTest", confidence,
	...
	private static int owedForPlayer(CommandContext<CommandSourceStack> ctx) {
		...
		var id = dev.lebron.staffcore.util.PlayerLookup.uuid(server, name);
		if (id.isEmpty()) {
		...
		for (var row : rows) {
			ctx.getSource().sendSuccess(() -> Icon.text("  " + row.describe(server) + "  ",
							row.kind() == dev.lebron.staffcore.storage.PendingActions.Kind.GIVE
	...
	private static int owedForgive(CommandContext<CommandSourceStack> ctx, boolean confirmed) {
		...
		var rows = StaffCore.pending().forPlayer(id.get()).stream()
				.filter(r -> r.kind() == dev.lebron.staffcore.storage.PendingActions.Kind.DEBIT)
				.toList();
		...
		int items = rows.stream().mapToInt(
				dev.lebron.staffcore.storage.PendingActions.Entry::count).sum();
	...
	private static int permsList(CommandContext<CommandSourceStack> ctx) {
		...
		if (groups.players.isEmpty()) {
			...
			groups.players.forEach((who, group) -> {
				String shown = dev.lebron.staffcore.util.PlayerLookup.display(server, who);
	...
	private static int permsSet(CommandContext<CommandSourceStack> ctx, String group) {
		...
		MinecraftServer server = ctx.getSource().getServer();
		if (server != null) {
			var profile = dev.lebron.staffcore.util.PlayerLookup.profile(server, player);
